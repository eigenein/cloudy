...
namespace Cloudy.Examples.Static.Pi.Master
{
    internal class ExampleMaster : Computing.StaticMaster
    {
            ...
            : base(port, threadsCount, new StarTopology())
        {
            ...
            SlaveLeft += 
                (sender, e) => Logger.Warn("Slave left: {0}", e.Value);
            ThreadsAllocated +=
            ...
            InterconnectionsSettingUp +=
                (sender, e) => Logger.Info("Interconnections Setting Up");
            ThreadsRun +=
                (sender, e) => Logger.Info("Threads run: {0}", e.Value);
            ConnectedNotification +=
                (sender, e) => Logger.Info("{0} has connected to the thread {1}", e.Value1, e.Value2);
        }
    }
}
