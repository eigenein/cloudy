...
namespace Cloudy.Examples.Static.AllReduce.Slave
{
    public class SlaveNode : AbstractSlaveNode<StarRank>
    {
        ...
        private static void Run(IEnvironment environment)
        {
            ...
            double pi = e.AllReduce(UserTags.Default, partOfPi, reductor,
                StarTopologyHelper.GetPeripherals(environment), StarTopologyHelper.GetCentral());
            Console.WriteLine("Pi is {0} in the {1} node.", pi, e.Rank);
            Logger.Info("AllReduce has finished.");
